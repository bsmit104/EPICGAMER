...
public class NoteController : MonoBehaviour
{
    ...
    void HideImmediate()
    {
        ...
        if (_tailSr  != null) _tailSr.enabled = false;
        if (_holdTail != null) _holdTail.SetActive(false);
    }
    ...
    void Update()
    {
        ...
        if (GameManager.Instance != null &&
            GameManager.Instance.SongTime > EndNote + _fallDuration + 2f)
        {
            HideImmediate();
            Destroy(gameObject);
            return;
    ...
    public HitRating TryHit(float songTime)
    {
        ...
        if (r != HitRating.None)
        {
            ...
            if (Type == NoteType.Hold)
            {
                ...
                // Hard scheduled destroy — fires unconditionally no matter what
                Invoke(nameof(ForceDestroy), _holdDuration + 0.3f);
            }
            else
            {
                _flashing = true;
...
//         // Hard timeout — nothing survives past its end time + fallDuration + 2s
//         if (GameManager.Instance != null &&
//             GameManager.Instance.SongTime > EndNote + _fallDuration + 2f)
//         {
//             Destroy(gameObject);
...
//         // Safety — destroy if stuck way past end time
